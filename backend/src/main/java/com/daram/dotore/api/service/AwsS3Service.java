...
public class AwsS3Service {
    ...
    /**
     * 프로필 이미지 업로드 S3버킷에 파일을 업로드하는 함수 파일, 경로, request(사용자 주소, String) 파일이 업로드된 경로(주소)를 반환
     */
    public String uploadFiles(MultipartFile multipartFile, String dirName,
        ProfileUpdateReq profileUpdateReq) throws IOException {
    ...
    /**
     * 작품 업로드 S3버킷에 파일을 업로드하는 함수 파일, 경로, request(사용자 주소, String) 파일이 업로드된 경로(주소)를 반환
     */
    public String uploadItem(MultipartFile multipartFile, String dirName, BigInteger tokenId,
    ...
    public String uploadMintedItem(File uploadFile, String filePath, BigInteger tokenId,
        String address) {
    ...
    /**
     * 민팅 전 파일 업로드 후 이미지 url 전달 S3버킷에 파일을 업로드하는 함수 파일이 업로드된 경로(주소)를 반환
     */
    ...
    public String uploadBeforeMint(File uploadFile, String dirName) {
        String fileName =
            dirName + "/" + UUID.randomUUID() + "&" + uploadFile.getName();   // S3에 저장된 파일 이름
        String uploadImageUrl = putS3(uploadFile, fileName); // s3로 업로드
        removeNewFile(uploadFile);
        return uploadImageUrl;
    }
